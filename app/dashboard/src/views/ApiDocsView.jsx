    ...
    items: [
      {
        ...
        desc: 'Step 1 of sign-in. Checks email + password, sends an OTP by SMS to the user\'s phone.',
        consumers: ['Dashboard'],
        req: { email: 'string', password: 'string' },
        res: { otp_sent: 'boolean', otp_length: 'number (6)', expires_in: 'seconds (300)', must_change_password: 'boolean' },
      },
      {
        method: 'POST', path: '/auth/verify-otp',
        desc: 'Step 2 of sign-in. Verifies the SMS code and returns the JWT. If must_change_password, the user is routed to create a new password.',
        consumers: ['Dashboard'],
        req: { email: 'string', code: 'string (6 digits)' },
        res: { access_token: 'jwt', user: 'object', must_change_password: 'boolean' },
      },
      {
        method: 'POST', path: '/auth/resend-otp',
        desc: 'Sends a fresh OTP by SMS if the previous one expired or did not arrive.',
        consumers: ['Dashboard'],
        req: { email: 'string' },
        res: { otp_sent: 'boolean' },
      },
      {
        method: 'POST', path: '/auth/set-password',
        desc: 'First sign-in, step 1. A freshly invited staff member sets their own password. Triggers a confirming OTP by SMS.',
        consumers: ['Dashboard'],
        req: { email: 'string', new_password: 'string' },
        res: { otp_sent: 'boolean', otp_length: 'number', expires_in: 'seconds' },
      },
      {
        method: 'POST', path: '/auth/confirm-password',
        desc: 'First sign-in, step 2. Confirms the new password with the SMS code, completes login.',
        consumers: ['Dashboard'],
        req: { email: 'string', code: 'string (6 digits)' },
        res: { access_token: 'jwt', user: 'object' },
      },
      {
        method: 'POST', path: '/staff/invite',
        desc: 'Admin invites a new staff member. Backend creates the account, generates a default password, and emails it to the member.',
        consumers: ['Dashboard'],
        req: { name: 'string', email: 'string', phone: 'string', role: 'admin|staff|stadium-rep', title: 'string', team: 'string' },
        res: { member: 'object' },
      },
    ],
  },
